    ...
    <div className="grid gap-4 lg:grid-cols-3">
      ...
      <section className="lg:col-span-2">
        ...
        {selected && <div className="space-y-4">
          <div className="card p-5"><div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between"><div><h2 className="text-xl font-bold">{personName(selected.patient)}</h2><p className="text-sm text-slate-500">Chair {selected.chair?.code || selected.chair?.chairNumber} • Schedule {selected.schedule?.code || selected.schedule?._id || selected.schedule}</p></div><StatusBadge status={selected.status} /></div><div className="mt-4 flex flex-wrap gap-2"><button className="btn-light" onClick={() => action('Patient checked in', () => sessionApi.checkIn(selected._id))}>Check In</button><button className="btn-primary" onClick={() => action('Treatment started', () => sessionApi.start(selected._id))}>Start Treatment</button></div></div>
