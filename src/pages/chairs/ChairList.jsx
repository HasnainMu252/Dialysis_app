...
export default function ChairList() {
  ...
  return <div className="space-y-5"><PageHeader title="Chair Status" subtitle={canCreate ? 'Admin/front desk can create chairs. Clinical teams update operational status only.' : 'View and update operational chair status. Chair creation is admin/front desk only.'} />
    ...
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{items.map((chair) => <div className="card p-3" key={chair._id}><div className="flex items-start justify-between gap-3"><div><div className="text-base font-bold">{chairCodeOf(chair)}</div><p className="text-sm text-slate-500">{chair.location || 'No location'}</p><p className="text-xs text-slate-400">{chair.conditionNotes}</p></div><StatusBadge status={chair.status} /></div><div className="mt-3 grid grid-cols-3 gap-1.5">{['available','reserved','cleaning','maintenance','out_of_order'].map((s) => <button key={s} className="btn-light text-xs" onClick={() => update(chair, s)}>{s}</button>)}</div></div>)}</div>{!items.length && <EmptyState message="No chairs found" />}</div>;
}
