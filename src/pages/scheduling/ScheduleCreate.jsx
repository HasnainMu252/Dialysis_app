...
export default function ScheduleCreate() {
  ...
  return <div className="space-y-5">
    ...
    <section className="card p-5"><div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-bold">Chair Grid</h2><p className="text-sm text-slate-500">{checked ? `${dateOnly(form.date)} • ${form.startTime}-${form.endTime}` : 'Check availability to enable selection.'}</p></div>{selectedChair && <div className="rounded-xl bg-blue-50 px-3 py-2 text-sm font-semibold text-blue-700">Selected {selectedChair}</div>}</div><div className="grid gap-2 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">{chairs.map((chair) => { const code = chairCodeOf(chair); const info = chairStates.get(code) || { state: chair.status, reason: chair.status }; const available = checked && info.state === 'available'; const color = available ? 'border-green-500 bg-green-50 hover:bg-green-100' : info.state === 'cleaning' ? 'border-yellow-500 bg-yellow-50' : info.state === 'out_of_order' || info.state === 'maintenance' ? 'border-slate-400 bg-slate-100' : 'border-red-400 bg-red-50'; return <button type="button" key={chair._id} disabled={!available} onClick={() => setSelectedChair(code)} className={`rounded-xl border p-2.5 text-left transition disabled:cursor-not-allowed ${color} ${selectedChair === code ? 'ring-2 ring-blue-400' : ''}`}><div className="flex items-center justify-between gap-1"><b className="truncate text-sm">{code}</b><span className={`h-2.5 w-2.5 shrink-0 rounded-full ${available ? 'bg-green-500' : info.state === 'cleaning' ? 'bg-yellow-500' : 'bg-red-400'}`} /></div><p className="mt-0.5 truncate text-[11px] text-slate-500">{chair.location || 'No location'}</p></button>; })}</div><button type="button" onClick={createSchedule} disabled={!selectedChair || loading} className="btn-primary mt-5 w-full md:w-auto">Schedule Patient</button></section>
  </div>;
}
