...
export default function ScheduleList() {
  ...
  return <div className="space-y-5"><PageHeader title="Schedules" subtitle="Current/upcoming schedules only. Completed and past schedules move to history. Chair date and time are clearly shown to avoid double booking." />
    ...
    <section className="card p-5"><h3 className="mb-3 font-bold">Past / Completed Schedule History</h3>{historyItems.length ? <><div className="grid gap-2 md:grid-cols-2">{histPage.paged.map((s) => <ScheduleCard key={s.id || s.code} schedule={s} onClick={() => view(s)} />)}</div><Pagination page={histPage.page} pageCount={histPage.pageCount} total={histPage.total} onPage={histPage.setPage} label="past schedules" /></> : <p className="text-sm text-slate-500">No past/completed schedules.</p>}</section>
  </div>;
}
